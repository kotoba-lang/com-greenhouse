(ns greenhouse.main
  "Kotodama WASM entrypoint for the Greenhouse clean-room actor (L4) — Clojure port.
  ...
  facts (`greenhouse.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "greenhouse")
(def tier "L4")
...
(def entity-specs
  [{:entity "Candidate"           :plural "candidates"           :id-prefix "greenhou_can"
    :fields [:firstName :lastName :email :canEmail]              :required [:firstName :lastName]
    :coerce {:canEmail :bool}                      :refs {}}
   {:entity "Application"        :plural "applications"        :id-prefix "greenhou_app"
    :fields [:candidateId :prospect :appliedAt :status :jobPostId] :required [:prospect :appliedAt]
    :coerce {:prospect :bool}          :refs {:candidateId "Candidate"}}
   {:entity "Job"                :plural "jobs"                :id-prefix "greenhou_job"
    :fields [:name :requisitionId :status]                 :required [:name :status]
    :coerce {}                       :refs {}}
   {:entity "Offer"              :plural "offers"              :id-prefix "greenhou_off"
    :fields [:applicationId :candidateId :jobId :status :startDate] :required [:status :startDate]
    :coerce {}                       :refs {:applicationId "Application" :candidateId "Candidate" :jobId "Job"}}
   {:entity "Scorecard"          :plural "scorecards"          :id-prefix "greenhou_sco"
    :fields [:applicationId :interviewerId :score :notes]        :required [:score :notes]
    :coerce {}                       :refs {:applicationId "Application"}}
   {:entity "ScheduledInterview" :plural "scheduledinterviews" :id-prefix "greenhou_sch"
    :fields [:applicationId :name :scheduledAt :videoConferencingUrl] :required [:name :scheduledAt]
    :coerce {}                       :refs {:applicationId "Application"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"greenhouse.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "greenhouse-compat" :tier tier :entities entities} 200])
...
(defn start! [] :greenhouse-compat/ready)
